...
assert_eq "json files has 2 files (no crop)" "$(json_len "$OUT" "files")" "2"
assert_eq "json files values are absolute" "$(json_all_abs "$OUT" "files")" "True"
assert_eq "json output has 2 files (no crop)" "$(json_len "$OUT" "output")" "2"
...
LINE_COUNT="$(printf '%s\n' "$OUT" | sed '/^$/d' | wc -l | tr -d ' ')"
assert_eq "text output has 2 lines" "$LINE_COUNT" "2"
...
assert_eq "crop files has 4 files" "$(json_len "$OUT" "files")" "4"
assert_eq "crop output has 4 files" "$(json_len "$OUT" "output")" "4"
